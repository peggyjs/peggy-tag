...
function callLocation(depth) {
  ...
  // Function name of the tag, e.g. "peggy".
  const fun = stack[depth - 1].getFunctionName();
  return new peggy.GrammarLocation(
    path,
    {
      offset: callsite.getPosition() + fun.length,
      line: callsite.getLineNumber(),
      column: callsite.getColumnNumber() + fun.length,
    }
  );
}
...
peg.withOptions = opts => (
   ...
   * @returns {function(string, peggy.ParserOptions): any} The parsing function.
   */
  function peg(strings, ...values) {
    return pegWithOptions(opts, strings, ...values);
  }
);
